const { MusicAndParty } = require("../model");
...
exports.create = async (req, res) => {
    try {
        ...
        const newMusicAndParty = new MusicAndParty(data);

        await newMusicAndParty.save();

        const datad = { ...newMusicAndParty._doc }

        handleResponse(res, datad, 201)
    } catch (error) {
        if (error.code === 11000) {
            handleError('This MusicAndParty already exists.', 400, res)
            return
        }
...
exports.find = async (req, res) => {
    try {
        ...
        const users = await MusicAndParty.find({ ...searchFilter })

        const getUsers = users.filter((user) => user.role !== 'admin')

        const totalCount = await MusicAndParty.countDocuments()
...
exports.findOne = async (req, res) => {
    try {
        const { id } = req.params;
        const user = await MusicAndParty.findOne({ _id: id })
...
exports.update = async (req, res) => {
    try {
        ...
        await MusicAndParty.updateOne({ _id: id }, data, { new: true })
